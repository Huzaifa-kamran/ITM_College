 ...
 */
(function ($) {
    'use strict'; $.fn.countdown = function (options, callback) {
        var thisEl = $(this); var settings = { 'date': null, 'format': null };
        if (options) { $.extend(settings, options); } function countdown_proc() {
            var eventDate = Date.parse(settings['date']) / 1000;
            var currentDate = Math.floor($.now() / 1000); if (eventDate <= currentDate) { callback.call(this); clearInterval(interval); }
            var seconds = eventDate - currentDate; var days = Math.floor(seconds / (60 * 60 * 24)); seconds -= days * 60 * 60 * 24;
            var hours = Math.floor(seconds / (60 * 60)); seconds -= hours * 60 * 60; var minutes = Math.floor(seconds / 60);
            seconds -= minutes * 60; if (days == 1) { thisEl.find(".timeRefDays").text("day"); }
            ...
            if (settings['format'] == "on") {
                ...
                var hours = (String(hours).length >= 2) ? hours : "0" + hours; var minutes = (String(minutes).length >= 2) ? minutes : "0" + minutes; var seconds = (String(seconds).length >= 2) ? seconds : "0" + seconds;
            ...
            else { alert("Invalid date. Here's an example: 12 Tuesday 2012 17:30:00"); clearInterval(interval); }
        } countdown_proc(); var interval = setInterval(countdown_proc, 1000);
    }
})(jQuery);
